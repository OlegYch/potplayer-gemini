...
class GeminiTest(using ExecutionEnv) extends Specification {
  ...
  "Hello" in {
    translateImpl("Hello", Nil, None, None, "Italian", keys, model).map(_ must beLike { case (_, _, Right("Ciao")) =>
      ok
    })
  }
  "many lines" in {
    translateImpl("Hello\nGoodbye", Nil, None, None, "Italian", keys, model).map(_ must beLike { case (_, _, Right("Ciao\nArrivederci")) =>
      ok
    })
  }
  "Hello with context" in {
    translateImpl("Hello bill", List("I'm bill" -> "joe"), None, None, "Italian", keys, model).map(_ must beLike {
      case (_, _, Right("Ciao joe")) =>
        ok
    })
  }
